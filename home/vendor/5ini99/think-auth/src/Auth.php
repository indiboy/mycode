...
-- ----------------------------
-- think_auth_rule，规则表，
-- id:主键，name：规则唯一标识, groupname：规则中文名称 status 状态：为1正常，为0禁用，condition：规则表达式，为空表示存在就验证，不为空表示按照条件验证
-- ----------------------------
...
class Auth
{
     ...
     *     array('userid'=>'用户id','groupid'=>'用户组id','groupname'=>'用户组名称','rules'=>'用户组拥有的规则id,多个,号隔开'),
     *     ...)
     */
    public function getGroups($userid)
    {
        ...
        // 执行查询
        $user_groups = Db::view($auth_group_access, 'userid,groupid')
            ->view($auth_group, 'groupname,rules', "{$auth_group_access}.groupid={$auth_group}.groupid", 'LEFT')
            ->where("{$auth_group_access}.userid='{$userid}' and {$auth_group}.checked='1'")
            ->select();
    ...
    protected function getAuthList($userid, $type)
    {
        ...
        if (2 == $this->config['auth_type'] && Session::has('_auth_list_' . $userid . $t)) {
            return Session::get('_auth_list_' . $userid . $t);
        }
        //读取用户所属用户组
        ...
        if (2 == $this->config['auth_type']) {
            //规则列表结果保存到session
            Session::set('_auth_list_' . $userid . $t, $authList);
        }
    ...
    protected function getUserInfo($userid)
    {
        ...
        // 获取用户表主键
        $_pk = is_string($user->getPk()) ? $user->getPk() : 'userid';
        if (!isset($userinfo[$userid])) {
            $userinfo[$userid] = $user->where($_pk, $userid)->find();
        }
